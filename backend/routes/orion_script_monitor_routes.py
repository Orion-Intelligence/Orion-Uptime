...
async def create_orion_script_monitor(request: CreateOrionScriptMonitorRequest, service: OrionScriptMonitorManager = Depends(get_orion_script_service)):
    return success_response(message=Messages.MONITOR_CREATED, data=await service.create_monitor(name=request.name, url=request.url, check_interval=request.check_interval, timeout=request.timeout, expected_response_time_ms=request.expected_response_time_ms, auth_profile_id=request.auth_profile_id))
