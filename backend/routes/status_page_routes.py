...
async def get_public_page(slug: str, service: StatusPageManager = Depends(get_status_page_service)):
    return success_response(message="Public status page retrieved successfully.", data=await service.get_public_page(slug))
    ...
    async def events():
        ...
        try:
            ...
            while not await request.is_disconnected():
                ...
                try:
                    ...
                    if asyncio.get_running_loop().time() - last_snapshot_at >= Intervals.PUBLIC_REFRESH_SECONDS:
                        try:
                            ...
                            yield _event("deleted", {"message": "This public monitor is no longer available."})
                            return
                ...
                page_changed = any(kind == "status_page" and entity_id in {page.id, None} for kind, entity_id in update.changed)
                monitor_changed = any(kind == "monitor" and entity_id == monitor_id for kind, entity_id in update.changed)
...
async def stream_public_page(slug: str, request: Request, service: StatusPageManager = Depends(get_status_page_service)):
    ...
    async def events():
        ...
        try:
            yield "retry: 1000\n\n"
            yield _event("snapshot", await service.build_public_response(page, _overviews_from(snapshot), snapshot["revision"]), snapshot["revision"])
            while not await request.is_disconnected():
                elapsed = asyncio.get_running_loop().time() - last_snapshot_at
                timeout = min(Intervals.KEEP_ALIVE_SECONDS, max(0.1, Intervals.PUBLIC_REFRESH_SECONDS - elapsed))
                try:
                    ...
                    if asyncio.get_running_loop().time() - last_snapshot_at >= Intervals.PUBLIC_REFRESH_SECONDS:
                        ...
                        yield _event("snapshot", await service.build_public_response(page, _overviews_from(current_snapshot), current_snapshot["revision"]), current_snapshot["revision"])
                        last_snapshot_at = asyncio.get_running_loop().time()
                        continue
                    yield ": keep-alive\n\n"
                    continue

                status_page_changed = any(kind == "status_page" and entity_id in {page.id, None} for kind, entity_id in update.changed)
                monitor_changed = any(kind == "monitor" and entity_id in page.monitor_ids for kind, entity_id in update.changed)
                ...
                yield _event("snapshot", await service.build_public_response(page, _overviews_from(await realtime_broker.get_snapshot()), update.revision), update.revision)
                last_snapshot_at = asyncio.get_running_loop().time()
        finally:
            realtime_broker.unsubscribe(queue)
...
@router.post("", response_model=SuccessResponse[StatusPageResponse], dependencies=[Depends(require_admin())])
async def create_page(request: CreateStatusPageRequest, service: StatusPageManager = Depends(get_status_page_service)):
    return success_response(message="Status page created successfully.", data=await service.create_page(request))


@router.get("", response_model=SuccessResponse[list[StatusPageResponse]], dependencies=[Depends(require_admin())])
async def list_pages(service: StatusPageManager = Depends(get_status_page_service)):
    return success_response(message="Status pages retrieved successfully.", data=await service.list_pages())


@router.get("/{page_id}", response_model=SuccessResponse[StatusPageResponse], dependencies=[Depends(require_admin())])
async def get_page(page_id: str, service: StatusPageManager = Depends(get_status_page_service)):
    return success_response(message="Status page retrieved successfully.", data=await service.get_page(page_id))


@router.put("/{page_id}", response_model=SuccessResponse[StatusPageResponse], dependencies=[Depends(require_admin())])
async def update_page(page_id: str, request: UpdateStatusPageRequest, service: StatusPageManager = Depends(get_status_page_service)):
    return success_response(message="Status page updated successfully.", data=await service.update_page(page_id, request))


@router.delete("/{page_id}", response_model=SuccessResponse[None], dependencies=[Depends(require_admin())])
async def delete_page(page_id: str, service: StatusPageManager = Depends(get_status_page_service)):
    ...
    return success_response(message="Status page deleted successfully.", data=None)
