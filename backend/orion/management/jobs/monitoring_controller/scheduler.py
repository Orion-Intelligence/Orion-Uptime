...
class MonitorScheduler:
    ...
    async def reconcile(self) -> None:
        try:
            ...
            for monitor in monitors:
                if isinstance(monitor, HeartbeatMonitorModel) and monitor.last_heartbeat_at is None:
                    continue
    ...
    def status(self) -> dict:
        ...
        return {"running": self._running, "workers": len(self._workers), "alive_workers": alive, "seconds_since_reconcile": None if self.last_reconcile_at is None else round(self.clock() - self.last_reconcile_at, 1), "last_reconcile_error": self.last_reconcile_error}
