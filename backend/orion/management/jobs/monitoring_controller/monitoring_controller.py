...
class MonitorManager:
    ...
    def _build_incident_reason(cls, monitor: MonitorModel, result) -> str:
        ...
        if status_code is not None and expected_status_code is not None and status_code != expected_status_code:
            return f"Expected HTTP {cls._http_status_label(expected_status_code)}, but received HTTP {cls._http_status_details(status_code)}."

        if status_code is not None:
            return f"Received HTTP {cls._http_status_details(status_code)}, but the configured response requirements were not satisfied."
    ...
    async def get_monitors_with_lookup(self) -> tuple[list[MonitorModel], dict[str, MonitorModel]]:
        ...
        return (monitors, {monitor.persisted_id: monitor for monitor in monitors})
    ...
    async def delete_monitor_history(self, monitor_id: str) -> None:
        ...
        status_pages = self.monitor_result_service.collection.database[Collections.STATUS_PAGES]
    ...
    async def process_heartbeat(self, monitor: HeartbeatMonitorModel) -> None:
        checked_at = datetime.now(UTC)
        await self.monitor_result_service.record_result(monitor_id=monitor.persisted_id, monitor_type=monitor.monitor_type, status=MonitorStatus.UP, status_code=None, response_time_ms=None, success=True, is_slow=False)

        state_result = await self.monitor_state_service.process_result(monitor_id=monitor.persisted_id, monitor_type=monitor.monitor_type, success=True, status_code=None, response_time_ms=None, checked_at=checked_at)
    ...
    def _get_monitor_service(self, monitor_type: MonitorType) -> MonitorRepository:
        try:
            ...
            raise ValueError(f"Unsupported monitor type: {monitor_type}") from exc
