...
class PingChecker:
    async def check(self, monitor: PingMonitorModel) -> HealthCheckResponse:
        ...
        timed_out = False
        try:
            process = await asyncio.create_subprocess_exec("ping", "-c", "1", "-W", str(monitor.timeout), "--", monitor.host, stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE)
            ...
            if process.returncode == 0:
                ...
                response_time_ms = await self._tcp_reachability_time(monitor.host, monitor.timeout)
                if response_time_ms is not None:
                    ...
                    error = (f"The target did not answer ICMP ping and no TCP connection could be established on ports 443, 53, or 80. {error_output}").strip()
        ...
        return HealthCheckResponse(url=monitor.host, status=status, status_code=None, response_time_ms=response_time_ms, success=success, is_slow=is_slow, error=error, timed_out=timed_out)
