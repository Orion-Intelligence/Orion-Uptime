...
class MonitorStateManager:
    ...
    async def get_or_create(self, monitor_id: str, monitor_type: MonitorType) -> MonitorStateModel:
        document = await self.collection.find_one({"monitor_id": monitor_id, "monitor_type": monitor_type})
        if document is None:
    ...
    async def process_result(self, monitor_id: str, monitor_type: MonitorType, success: bool, status_code: int | None, response_time_ms: int | None, checked_at: datetime) -> MonitorStateResult:
        ...
        await self.collection.update_one({"monitor_id": state.monitor_id, "monitor_type": state.monitor_type}, {"$set": {"status": state.status, "consecutive_failures": state.consecutive_failures, "consecutive_successes": state.consecutive_successes, "last_checked_at": state.last_checked_at, "last_status_code": state.last_status_code, "last_response_time_ms": state.last_response_time_ms}})
