...
class DashboardManager:
    ...
    async def collect_snapshot_sections(self) -> tuple[DashboardSummaryResponse, list[DashboardIncidentResponse], list[DashboardActivityResponse], list[MonitorOverviewResponse]]:
        monitors, monitor_map = await self.monitor_service.get_monitors_with_lookup()
        monitor_ids = [monitor.id for monitor in monitors if monitor.id is not None]
        first_check_times, incidents_by_monitor, recent_incidents, latest_results, open_incidents, average_response_time = await asyncio.gather(
        ...
        incidents = self._build_recent_incidents(recent_incidents, monitor_map)
        activity = self._build_recent_activity(latest_results[:ACTIVITY_LIMIT], monitor_map)
        return summary, incidents, activity, overviews

    async def _overviews_for(self, monitors) -> list[MonitorOverviewResponse]:
        monitor_ids = [monitor.id for monitor in monitors if monitor.id is not None]
        ...
        return self._build_overviews(monitors, first_check_times, incidents_by_monitor)

    @staticmethod
    def _build_summary(monitors, monitor_map, latest_results, open_incidents, average_response_time, overviews) -> DashboardSummaryResponse:
    ...
    @staticmethod
    def _build_recent_incidents(incidents, monitor_map) -> list[DashboardIncidentResponse]:
        results = []
    ...
    @staticmethod
    def _build_recent_activity(results, monitor_map) -> list[DashboardActivityResponse]:
        activities = []
    ...
    @staticmethod
    def _build_overviews(monitors, first_check_times, incidents_by_monitor) -> list[MonitorOverviewResponse]:
        ...
        for monitor in monitors:
            ...
            if monitor.status != MonitorStatus.UNKNOWN and first_check_at is not None:
                period_start = max(DashboardManager._as_utc(first_check_at), now - timedelta(days=7))
                period_end = max(period_start, DashboardManager._as_utc(measurement_end))
                measured_seconds = (period_end - period_start).total_seconds()
                downtime_seconds = sum(DashboardManager._incident_overlap_seconds(incident.started_at, incident.resolved_at, period_start, period_end) for incident in incidents)
            ...
            if monitor.status == MonitorStatus.UP:
                ...
                if uptime_started_at is not None:
                    current_uptime_seconds = DashboardManager._duration_seconds(uptime_started_at, measurement_end)
