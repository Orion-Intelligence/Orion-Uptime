...
class AccessTokenCookieManager:
    ...
    async def authenticate_profile(self, profile: AuthProfileModel) -> tuple[str, int]:
        try:
            ...
            raise AuthTokenError(f"Login request failed for profile '{profile.name}': {exc}") from exc

        if not response.is_success:
            raise AuthTokenError(f"Login returned HTTP {response.status_code}. Auth profile was not created.", status_code=response.status_code)
    ...
    def cache_token(self, profile_id: str, token: str) -> None:
        self._cache[profile_id] = CachedAccessToken(value=token, expires_at=self.clock() + TOKEN_CACHE_TTL_SECONDS)
