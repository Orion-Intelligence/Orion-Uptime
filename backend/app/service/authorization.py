...
from app.modules.auth_manager.auth_manager import AuthManager, password_service, refresh_token_service, revoked_access_tokens
...
def get_auth_service(engine: AIOEngine = Depends(get_engine)) -> AuthManager:
    return AuthManager(
        engine=engine,
        password_manager=password_service,
        jwt_service=app_dependency,
        refresh_token_manager=refresh_token_service,
    )


def development_environment() -> bool:
    load_dotenv()
    return os.getenv("APP_ENV", "production").lower() in {
        "development",
        "local",
        "test",
    }
